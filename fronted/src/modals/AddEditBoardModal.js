...
import { updateBoard, createBoard } from "../redux/boardsSlice";
...
export default function AddEditBoardModal({ type, setIsBoardModalOpen }) {
  ...
  const [newColumns, setNewColumns] = useState([
    { name: "Todo", tasks: [], key: uuidv4()  },
    { name: "Doing", tasks: [], key: uuidv4() },
  ]);
  ...
  const onChange = (id, newValue) => {
    setNewColumns((prevState) => {
      ...
      const column = newState.find((col) => col.key === id);
  ...
  const onDelete = (id) => {
    setNewColumns((prevState) => prevState.filter((el) => el.key !== id));
  };
  ...
  const onSubmit = (type) => {
    ...
    if (type === "add") {
      dispatch(createBoard({ name, columns: newColumns, isActive: false }))
      dispatch(boardsSlice.actions.addBoard({ name, newColumns }));
    } else {
      dispatch(updateBoard({ ...board, name, columns: newColumns, isActive: false }));
